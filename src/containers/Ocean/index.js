    ...
    initThree = () => {
        ...
        loader.load(islandModel, mesh => {
            ...
            const createHeartShape = () => {
                ...
                // 从尖端开始绘制
                heartShape.moveTo(0, 0.5);  // 从尖端开始，向上绘制
                // 左侧曲线
                heartShape.bezierCurveTo(-0.5, 1, -1, 0.3, 0, -0.5);  // 调整控制点确保尖端朝下
                // 右侧曲线
                heartShape.bezierCurveTo(1, 0.3, 0.5, 1, 0, 0.5);  // 右侧曲线对称
                return heartShape;
            };
            // 创建一个爱心几何体
            ...
            const extrudeSettings = {
                ...
                bevelThickness: 0.02,  // 倒角的厚度
                bevelSize: 0.05,  // 倒角的大小
                bevelOffset: 0,  // 倒角偏移
                bevelSegments: 3  // 倒角的细分数
            };
            //const heartGeometry = new THREE.ShapeGeometry(heartShape);
            const heartGeometry = new THREE.ExtrudeGeometry(heartShape, extrudeSettings);

            const heartMaterial = new THREE.MeshStandardMaterial({
                color: 0xff0066,  // 爱心的基本颜色
                metalness: 0.8,   // 金属感
                roughness: 0.3,   // 粗糙度
                emissive: 0xff6699, // 发光效果
                emissiveIntensity: 0.4,
                side: THREE.DoubleSide,  // 使爱心两面可见
            });

            // 创建材质
            //const heartMaterial = new THREE.MeshBasicMaterial({color: 0xff0000, side: THREE.DoubleSide});
            const hearts = [];
            const chairPosition = new THREE.Vector3(-100, 50, -30);

            // 创建爱心并添加动画
            const createHeart = () => {
                const heart = new THREE.Mesh(heartGeometry, heartMaterial);
                heart.scale.set(0.05, 0.05, 0.05); // 初始大小
                heart.position.set(0, 0, 0); // 初始位置在岛屿正上方
                hearts.push(heart);
                mesh.scene.add(heart);

                // 动画逻辑：逐渐向上移动并放大，最后消失
                const riseTween = new TWEEN.Tween(heart.position)
                    .to({ y: 10 }, 15000) // 上升动画持续3秒
                    .easing(TWEEN.Easing.Quadratic.Out);

                const scaleTween = new TWEEN.Tween(heart.scale)
                    .to({ x: 0.5, y: 0.5, z: 0.5 }, 15000) // 缩放动画持续3秒
                    .easing(TWEEN.Easing.Quadratic.Out)
                    .onComplete(() => {
                        mesh.scene.remove(heart); // 动画完成后移除
        ...
        document.querySelectorAll('.point').forEach(item => {
            item.addEventListener('click', event => {
                ...
                switch (className) {
                    case 'label-0':
                        Animations.animateCamera(camera, controls, {x: -15, y: 80, z: 60}, {
                            x: 0,
                            y: 0,
                            z: 0
                        }, 1600, () => {
                        ...
                        break;
                    case 'label-1':
                        Animations.animateCamera(camera, controls, {x: -20, y: 10, z: 60}, {
                            x: 0,
                            y: 0,
                            z: 0
                        }, 1600, () => {
                        ...
                        break;
                    case 'label-2':
                        Animations.animateCamera(camera, controls, {x: 30, y: 10, z: 100}, {
                            x: 0,
                            y: 0,
                            z: 0
                        }, 1600, () => {
                        ...
                        break;
                    default:
                        Animations.animateCamera(camera, controls, {x: 0, y: 40, z: 140}, {
                            x: 0,
                            y: 0,
                            z: 0
                        }, 1600, () => {
        ...
        return (
            <div className='ocean'>
                ...
                <div className="point point-0">
                    ...
                    <div
                        className="text">灯塔：矗立在海岸的岩石之上，白色的塔身以及红色的塔屋，在湛蓝色的天空和深蓝色大海的映衬下，显得如此醒目和美丽。
                    </div>
                </div>
                <div className="point point-1">
                    <div className="label label-1">2</div>
                    <div
                        className="text">小船：梦中又见那宁静的大海，我前进了，驶向远方，我知道我是船，只属于远方。这一天，我用奋斗作为白帆，要和明天一起飘扬，呼喊。
                    </div>
                </div>
                <div className="point point-2">
                    <div className="label label-2">3</div>
                    <div
                        className="text">沙滩：宇宙展开的一小角。不想说来这里是暗自疗伤，那过于矫情，只想对每一粒沙子，每一朵浪花问声你们好吗
                    </div>
                </div>
                <div className="point point-3">
                    <div className="label label-3">4</div>
                    <div
                        className="text">飞鸟：在苍茫的大海上，狂风卷集着乌云。在乌云和大海之间，海燕像黑色的闪电，在高傲地飞翔。
                    </div>
                </div>
